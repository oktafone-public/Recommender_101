...
class AlternatingLeastSquares:
    def __init__(self,c,d,runPerRound):
        self.c = c
        self.d = d
    ...
    def retrain(self,data,oldShape,coords):
        ...
        for n in range(self.runPerRound):
            for i in {x[0] for x in coords}:
                self.U = self.improveRow(M,self.U,self.V,i)
            for j in {x[1] for x in coords}:
                self.V = self.improveCol(M,self.U,self.V,j)
